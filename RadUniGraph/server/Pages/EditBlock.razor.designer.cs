...
{
    public partial class EditBlockComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            var uniSqlGetBlockByBlockIdResult = await UniSql.GetBlockByBlockId(int.Parse($"{BlockId}"));
            block = uniSqlGetBlockByBlockIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(UniBlocksGraph.Models.UniSql.Block args)
        {
            try
            {
                var uniSqlUpdateBlockResult = await UniSql.UpdateBlock(int.Parse($"{BlockId}"), block);
                DialogService.Close(block);
            }
            catch (Exception uniSqlUpdateBlockException)
            {
                    NotificationService.Notify(NotificationSeverity.Error, $"Error", $"Unable to update Block");
            }
        }
